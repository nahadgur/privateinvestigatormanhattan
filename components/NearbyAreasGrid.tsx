...
export function NearbyAreasGrid({ cityName, serviceSlug, serviceName, initialVisible = 10 }: NearbyAreasGridProps) {
  ...
  const description = serviceName
    ? `Looking for ${serviceName.toLowerCase()} near ${cityName}? Our licensed investigators serve clients across ${cityName} and surrounding areas. We will match you with the right PI for your case.`
    : `Our licensed investigators serve clients from across the surrounding area. If you are based in any of the nearby areas listed below, we can match you with a licensed PI for your case.`;
  ...
  return (
    <section className="mb-16">
      ...
      <div className="mt-6 prose prose-sm max-w-none text-gray-500">
        <p>
          Clients from {areas.slice(0, 5).join(', ')}, and other areas around {cityName} regularly use our service to find licensed private investigators.{' '}
          {serviceName
            ? `If you need ${serviceName.toLowerCase()} and are based in or near ${cityName}, our licensed investigators offer a free confidential consultation to discuss your case.`
            : `All of our ${cityName} partner investigators are NYS-licensed, fully insured, and experienced in Manhattan-area cases.`
          }
        </p>
      </div>
    </section>
